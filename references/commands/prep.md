...
| **Bar raiser — CV spine** | CV walkthrough is the spine; questions emerge organically from each role period rather than as standalone prompts. Interviewer moves chronologically through the candidate's history, probing each period for achievement, rating, reason for leaving, and "above and beyond" evidence. Confirmed format for Revolut Bar Raiser (per Anam Waseem + Shane). Key difference from standard bar raiser: the candidate cannot choose which story to deploy — they must defend whatever is in front of them for that period. Sustained probing within a single period (10-20 min on one role) is expected. **Run Bar Raiser CV-Spine Simulation UX in `mock`.** | Credibility + Differentiation weighted highest. Add **Period Depth** sub-dimension: did the candidate sustain quality when probed repeatedly within a single role period, or did answers thin out after the first response? |
